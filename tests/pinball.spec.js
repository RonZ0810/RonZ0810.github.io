...
test('XP drafts pause safely and resolve correctly at encounter and level boundaries', async ({ page }) => {
  ...
  const before = await page.evaluate(() => FLIPSTRIKE.run.time); await page.locator('.upgrade-card').first().click(); const transition = await page.evaluate(() => ({ phase: FLIPSTRIKE.phase, timerActive: FLIPSTRIKE.isMainTimerActive(), encounter: FLIPSTRIKE.run.encounter, time: FLIPSTRIKE.run.time })); expect(transition).toEqual({ phase: 'encounterTransition', timerActive: false, encounter: 0, time: before }); await expect(page.locator('#transition-title')).toHaveText('PHASE 2 / 2'); await expect.poll(() => page.evaluate(() => FLIPSTRIKE.enemies.length)).toBeGreaterThan(0); const expected = await page.evaluate(() => FLIPSTRIKE.run.challenge.encounters[1]); expect(await page.evaluate(() => FLIPSTRIKE.enemies.map((enemy) => enemy.getUserData().id))).toEqual(expected); expect(await page.evaluate(() => FLIPSTRIKE.run.time)).toBeGreaterThanOrEqual(before + 7.9); await page.waitForTimeout(650); expect(await page.evaluate(() => FLIPSTRIKE.enemies.map((enemy) => enemy.getUserData().id))).toEqual(expected);
});
