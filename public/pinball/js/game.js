(async function () {
  ...
  class GameDirector {
    ...
    installCardTableElement(effect) { const definition = this.cardElementDefinition(effect); if (definition) this.createTableElement(structuredClone(definition), this.tableElements.length); }
    ...
    buildTable(savedState = null) {
      const layout = D.tableById[this.levelSpec().layoutId] || D.TABLE_MODULES[0], saved = new Map((savedState || []).map((item) => [item.id, item])); this.run.layoutId = layout.id;
      const definitions = [...layout.elements.map((definition, index) => ({ ...definition, key: `base-${index}` })), ...this.cardTableElements(), ...this.bossTableElements().map((definition, index) => ({ ...definition, key: `boss-${index}` }))]; definitions.forEach((definition, index) => this.createTableElement(structuredClone(definition), index, saved.get(`${layout.id}:${definition.key}`)));
    ...
    createTableElement(def, index, saved = null) {
      ...
      else if (def.type === "ramp") { def.rails.forEach((line) => edge(line, .65, "rampRail")); const [x, y, vx, vy] = def.exit, body = this.world.createBody({ position: P.Vec2(x / SCALE, y / SCALE) }), fixture = body.createFixture(P.Circle(24 / SCALE), { isSensor: true }); body.setUserData({ type: "rampExit", elementId: id, launch: [vx, vy] }); element.bodies.push(body); element.fixtures.push({ fixture, solid: false }); }
    ...
    spawnEnemyInstance(item, index = this.enemies.length) {
      const def = D.enemyById[item.id]; if (!def) return null; const scale = this.run.endless ? 1 + (this.run.displayLevel - 101) * .025 : 1 + Math.max(0, this.run.level - 1) * .012, maxHp = item.maxHp || Math.round(def.hp * scale), position = item.p || { x: (160 + (index % 4) * 135) / SCALE, y: (260 + Math.floor(index / 4) * 125) / SCALE }, body = this.world.createKinematicBody({ position: P.Vec2(position.x, position.y) }), restored = item.state || {}, eliteModifiers = restored.eliteModifiers || (this.levelSpec().elite ? [...this.levelSpec().eliteModifiers] : []);
      const state = { type: "enemy", id: item.id, hp: item.hp ?? maxHp, maxHp, phase: item.phase || 1, t: index * .71, flash: 0, invulnerable: false, anchor: { x: position.x, y: position.y }, direction: index % 2 ? -1 : 1, behaviorClock: index * .43, actionClock: 2.2 + index * .37, teleportIndex: index, shieldFacing: index % 2 ? -1 : 1, armorPlates: def.armor ? Math.max(1, def.armor + Math.floor(def.tier / 3)) : 0, eliteModifiers, linkTarget: null, pulse: 0, ...restored };
    ...
    onContact(contact) {
      ...
      if (data.type === "rampExit") { const [vx, vy] = data.launch; ball.setLinearVelocity(P.Vec2(vx, vy)); element.pulse = .5; this.addXp(3); }
      if (data.type === "kicker" && !element.captures.some((capture) => capture.ball === ball)) { ball.setGravityScale(0); element.captures.push({ ball, remaining: .2 }); element.pulse = .4; }
    ...
    checkBossPhase(enemy) {
      ...
      this.run.bossPhase = target; data.phase = target; this.projectiles = []; this.run.time += 12; this.run.balls = Math.min(7, this.run.balls + 1); this.setBossTablePhase(); this.transformTable(target); this.float(`PHASE ${target}`, enemy, 0xff7699, 30); this.audio.tone(95, .4, .08);
    }
    killEnemy(enemy) {
    ...
    encounterCleared() {
      const spec = this.levelSpec(); if (this.run.encounter + 1 < spec.encounters.length) { this.run.encounter++; this.run.time += spec.continuous ? 5 : 10; if (D.tableById[spec.layoutId]?.transforms) this.transformTable(this.run.encounter + 1); setTimeout(() => { this.spawnEncounter(); this.toast(spec.continuous ? "ZONE ASCENDING" : "ENCOUNTER OPEN"); }, 500); } else this.levelComplete();
    }
    ...
    chooseCard(id) {
      ...
      this.run.cards[id] = (this.run.cards[id] || 0) + 1; if (card.category === "consumable") this.run.consumables.push(id); if (card.category === "table" && this.effectCount(card.effect) === 1) this.installCardTableElement(card.effect); if (!S.progress.discovered.includes(id)) { S.progress.discovered.push(id); S.saveProgress(); }
    ...
    updateEnemies(dt) {
      ...
      enemies.forEach((body, index) => {
        if (!this.enemies.includes(body)) return; const data = body.getUserData(), def = D.enemyById[data.id], pos = body.getPosition(), tier = def.tier || 8, haste = data.eliteModifiers?.includes("haste") ? 1.28 : 1, speed = (1.3 + (def.speed || 25) / 35) * def.escalation.speed * haste, t = (data.t += dt * speed), anchor = data.anchor || vecOut(pos), range = (42 + tier * 4) / SCALE; data.behaviorClock += dt; data.actionClock -= dt; data.flash = Math.max(0, data.flash - dt); data.pulse = Math.max(0, data.pulse - dt);
        ...
        targetX = clamp(targetX, 1.65, 10.35); targetY = clamp(targetY, 3.2, 14.2); if (def.movement !== "teleport") body.setLinearVelocity(P.Vec2((targetX - pos.x) * 3.2, (targetY - pos.y) * 3.2));
      });
    ...
    updateTable(dt) {
      this.tableElements.forEach((element) => {
        ...
        if (element.captures.length) element.captures = element.captures.filter((capture) => { if (!this.balls.includes(capture.ball)) return false; capture.remaining -= dt; const center = P.Vec2(def.x / SCALE, def.y / SCALE); capture.ball.setTransform(center, 0); capture.ball.setLinearVelocity(P.Vec2(0, 0)); if (capture.remaining > 0) return true; capture.ball.setGravityScale(1); capture.ball.setLinearVelocity(P.Vec2(def.launch[0], def.launch[1])); return false; });
      });
    }
    updateAttack(dt) {
    ...
    renderTable(biome) {
      this.tableElements.forEach((element) => {
        ...
        else if (def.type === "ramp") { def.rails.forEach((line) => art.moveTo(line[0], line[1]).lineTo(line[2], line[3]).stroke({ width: 7, color: biome.accent, alpha: .62 * alpha })); const [x, y, vx] = def.exit; art.poly([x - 12, y + 10, x, y - 10, x + 12, y + 10]).fill({ color: vx < 0 ? 0xff93c9 : biome.accent, alpha: .72 * alpha }); }
        else if (def.type === "kicker") { art.circle(def.x, def.y, def.radius + pulse * 8).fill({ color: 0x07151d, alpha: .9 * alpha }).stroke({ width: 5, color: biome.accent, alpha: .8 * alpha }); art.circle(def.x, def.y, 8).fill({ color: biome.accent, alpha: .7 * alpha }); }
    ...
    renderEnemyStatuses() {
      this.enemies.forEach((body) => {
        const data = body.getUserData(), def = D.enemyById[data.id], p = body.getPosition(), x = p.x * SCALE, y = p.y * SCALE, radius = def.radius || 62, sprite = this.actorSprites.get(body); if (sprite && data.invulnerable) sprite.alpha = .24;
